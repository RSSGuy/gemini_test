        ...
        child: Builder(
          builder: (BuildContext newContext) {
            return ListTile(
              ...
              onTap: () {
                context.go(route);
                // We use the newContext from the Builder to find the Scaffold.
                // `maybeOf` is safer than `of` because it returns null instead of throwing
                // an error if no Scaffold is found (e.g., on a wide screen).
                if (Scaffold.maybeOf(newContext)?.isDrawerOpen ?? false) {
                  // `closeDrawer` is the recommended way to close a drawer.
